...
Gen_model = 1; % as in the main tutorial code, many parameters can be adjusted
               ...
               % function starting on line 796. This includes, among
               % others (similar to in the main tutorial script):

% prior beliefs about context (d): alter line 853

% beliefs about hint accuracy in the likelihood (a): alter lines 973-975

% to adjust habits (e), alter line 1132
...
% on line 797), but see main tutorial script for more complete walk-through
...
if isfield(MDP,'d')
    for factor = 1:numel(d)
        ...
        d_complexity{factor} = spm_wnorm(d{factor});
    end 
end 

if isfield(MDP,'a')
    % complexity of a maxtrix concentration parameters
    for modality = 1:numel(a)
        a_prior{modality} = MDP.a{modality};
        a_complexity{modality} = spm_wnorm(a{modality}).*(a{modality} > 0);
    end
end  
...
for t = 1:T % loop over time points  
    ...
    for policy = 1:NumPolicies
        ...
        % mapping between hidden states and observations (minimize
        % amgibuity), and bring about preferred outcomes (minimize risk)"?
        ...
        for timestep = t:horizon
            ...
            % calculate ambiguity term (expected entropy of likelihood
            % mapping)
            Gintermediate(policy) = Gintermediate(policy) + G_ambiguity(a(:),Expected_states(:));
    ...
    % loop over policy selection using variational updates to beta to
    ...
    for ni = 1:Ni 
        ...
        dFd_beta = posterior_beta - beta + beta_update; % beta gradient w.r.t F
        posterior_beta = posterior_beta - dFd_beta/2;
...
% expected entropy of likelihood mapping (the ambiguity term in expected free energy)  
function G = G_ambiguity(A,s)
...
% a maximimally high value:

    alpha = 512; % no randomness in action selection
