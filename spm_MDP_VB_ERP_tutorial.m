function [x,y,ind,xx_yy] = spm_MDP_VB_ERP_tutorial(MDP,FACTOR,T)
...
for m = 1:numel(MDP)
    ...
    for k = 1:Ne
        ...
        if isfield(MDP,'mdp')
            y{k}   = spm_MDP_VB_ERP_tutorial(MDP(m).mdp(k),f2,1);
        else
            y{k}   = [];
        end
    end
...
ylabel('Depolarisation'),ylim([-.2 1.1]) %spm_axis tight
grid on, %set(gca,'XTick',(1:(length(t)/Nb))*Nb*dt)

subplot(4,1,4), plot(t,yy(end,:))
title('Local field potentials (Level 1)','FontSize',16)
ylabel('Depolarisation'),ylim([-1 1]) %spm_axis tight
grid on, %set(gca,'XTick',(1:(length(t)/Nb))*Nb*dt)
