...
rng('default') % This sets the random number generator to produce different 
...
% If Sim = 2, simulate multiple trials where the left context is active ([1 0])
            
% If Sim = 3, simulate reversal learning, where the left context is active ([1 0])
            % in the first half and then reverses in the second half ([0 1])
...
rs = 2; % By default we set this to 4, but try changing its value to 
...
% policy selection less deteriministic. We do not include this for our 
% example simulations, but could be set as follows:
...
% a high value:
    ...
    alpha = 128; % Any positive number. 1 is very low, 20 is fairly high; 
                 % an extremely high value can be used to specify
                 % deterministic action (e.g., 128)
...
% the spm_MDP_VB_X script (and in the slightly modified spm_MDP_VB_X_tutorial 
% script we provide in this tutorial).
...
% or expected precision of expected free energy over policies:
...
alpha = 8; % specify a lower action precision (8) than the prior value (16)
la = 1;   % keep loss aversion at a value of 1
rs = 4;   % specify higher risk-seeking (4) than the prior value (2)
...
rs_sequence = [1 5];   % specify different true risk-seeking values (prior = 2)
...
% The pxp value is the protected exceedence probability, which will provide
% a probability of each model being the best-fit model
...
% In this case, the correlations appear quite high for rs and eta, and moderate for
% alpha. Note, however, that a wider range of values should be simulated to
% confirm recoverability in actual studies.
...
% in estimated parameters. PEB uses a general linear model, which also 
% allows testing evidence for individual difference effects (e.g., age, 
% gender).
